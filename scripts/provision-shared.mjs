// Provision SundayQuiz INTO the shared SundayChess project, using a
// dedicated `quiz` Postgres schema. Applies migrations, exposes the schema
...
async function main() {
  ...
  // 2. Expose `quiz` to PostgREST (additive — keep public + graphql_public).
  ...
  const schemas = String(cfg.db_schema || "public, graphql_public")
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  if (!schemas.includes("quiz")) schemas.push("quiz");
  ...
  const env = [
    ...
    `NEXT_PUBLIC_BASE_URL=https://quiz.sundaysuite.app`,
    "",
  ].join("\n");
